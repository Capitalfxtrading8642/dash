...
export function capitalfxtrading_Router(request) {
   ...
   if (data) {
       ...
       // Render item page 
       return ok("capitalfxtrading-page", data, seoData); 
   }
...
export function capitalfxtrading_SiteMap(sitemapRequest) {
   ...
   const siteMapEntries = Object.keys(peopleData).map((name)=>{
                               ...
                               entry.pageName = "capitalfxtrading-page";		// The name of the page in the Wix editor to render
                               entry.url = "/capitalfxtrading/" + name ;			// Relative URL of the page
